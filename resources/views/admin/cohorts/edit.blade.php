    ...
    <div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.cohorts.update', $cohort) }}"
            ...
            <div>
                ...
                <input name="name" value="{{ old('name', $cohort->name) }}"
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input name="code" value="{{ old('code', $cohort->code) }}"
                        class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-300 focus:ring focus:ring-emerald-100"
                        required>
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-900">Location</label>
                    <input name="location" value="{{ old('location', $cohort->location) }}"
                        class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-300 focus:ring focus:ring-emerald-100">
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="date" name="start_date"
                        value="{{ old('start_date', optional($cohort->start_date)->format('Y-m-d')) }}"
                        class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-300 focus:ring focus:ring-emerald-100"
                        required>
                </div>
                <div>
                    ...
                    <input type="date" name="end_date"
                        value="{{ old('end_date', optional($cohort->end_date)->format('Y-m-d')) }}"
                        class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-300 focus:ring focus:ring-emerald-100">
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <select name="status"
                        class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-300 focus:ring focus:ring-emerald-100">
                        @foreach (['open', 'progress', 'upcoming', 'active', 'completed'] as $status)
                ...
                <div>
                    ...
                    <input type="number" name="capacity" value="{{ old('capacity', $cohort->capacity) }}"
                        class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-300 focus:ring focus:ring-emerald-100"
                        min="1">
                </div>
            </div>
            <div>
                ...
                <textarea name="description" rows="3"
                    class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-300 focus:ring focus:ring-emerald-100">{{ old('description', $cohort->description) }}</textarea>
            </div>
