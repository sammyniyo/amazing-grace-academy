...
@section('content')
    ...
    {{-- REGISTRATION FORM (only when intake is open or in progress) --}}
        ...
        <div
            ...
            @if (!$registrationOpen)
                ...
                <div class="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-8 text-center">
                    <div class="text-amber-700 font-display text-xl font-semibold">Registration is currently closed</div>
                    <p class="mt-2 text-sm text-amber-800 max-w-md mx-auto">
                        We are not accepting new registrations right now. Please check back later for the next intake, or
                        follow us for announcements.
                    </p>
                    <p class="mt-4 text-xs text-amber-600">You can still <a href="{{ route('contact') }}"
                            class="font-semibold underline hover:no-underline">contact us</a> to express your interest.</p>
                </div>
            @else
                ...
                <form class="mt-8 grid gap-4 md:grid-cols-2" method="POST" action="{{ route('register.submit') }}">
                    ...
                    @if (session('error'))
                        <div
                            class="md:col-span-2 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 font-medium">
                            {{ session('error') }}
                        </div>
                    @endif
                    @if ($errors->any())
                        <div
                            class="md:col-span-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
                    ...
                    <div>
                        <label class="text-sm font-medium text-ink-900">Full name <span
                    ...
                    <div class="md:col-span-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center gap-2 text-xs text-ink-500">
                            <span class="h-2 w-2 rounded-full bg-sage-500"></span> Free training • Saturdays • Kigali
                            Bilingual
                            Church
                        </div>
