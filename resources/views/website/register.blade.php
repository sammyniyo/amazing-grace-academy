...
@section('content')
    ...
    <section class="border-b border-ink-100 bg-white/80 backdrop-blur-sm sticky top-[72px] z-30" aria-label="Registration progress">
        <div class="mx-auto max-w-4xl px-4 py-3 sm:px-6">
            <div class="flex items-center justify-center gap-2 sm:gap-6 text-xs sm:text-sm font-medium text-ink-500">
                <span class="flex items-center gap-2" id="step-indicator-1">
                    <span class="registration-step flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-sage-600 text-white font-semibold" data-step="1" aria-current="step">1</span>
                ...
                <span class="flex items-center gap-2" id="step-indicator-2">
                    <span class="registration-step flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 border-ink-200 text-ink-400" data-step="2">2</span>
                    <span>Course choice</span>
                </span>
                <span class="text-ink-300" aria-hidden="true">→</span>
                <span class="flex items-center gap-2" id="step-indicator-3">
                    <span class="registration-step flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 border-ink-200 text-ink-400" data-step="3">3</span>
    ...
    <section class="mx-auto max-w-3xl px-6 pb-20">
        <div class="reveal soft-card p-10 bg-gradient-to-br from-white via-sage-50/40 to-gold-50/30 border border-sage-200/80">
            @if (!$registrationOpen)
                @if (session('error'))
                    <div class="mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 font-medium">
                        {{ session('error') }}
                    </div>
                @endif
                <div class="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-8 text-center">
                    ...
                    <p class="mt-2 text-sm text-amber-800 max-w-md mx-auto">
                        We are not accepting new registrations right now. Check back for the next intake or follow us for announcements.
                    </p>
                    <p class="mt-4 text-xs text-amber-600">
                        You can still <a href="{{ route('contact') }}" class="font-semibold underline hover:no-underline focus:outline-none focus:ring-2 focus:ring-amber-400 rounded">contact us</a> to express your interest.
                    </p>
                </div>
            @else
                {{-- Multi-step form --}}
                <form id="registration-form" class="mt-6" method="POST" action="{{ route('register.submit') }}">
                    ...
                    <div id="registration-step-1" class="registration-step-panel space-y-4" data-step="1">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                <label for="reg-name" class="text-sm font-medium text-ink-900">Full name <span class="text-sage-600">*</span></label>
                        ...
                        <p class="text-xs text-ink-500">Provide at least one of phone or email so we can confirm your registration.</p>
                        <div class="pt-2">
                            <button type="button" class="registration-next rounded-xl bg-sage-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-sage-700 focus:outline-none focus:ring-2 focus:ring-sage-500 focus:ring-offset-2" data-next="2">Continue to course choice</button>
                        </div>
                    </div>
                    ...
                    <div id="registration-step-2" class="registration-step-panel hidden space-y-4" data-step="2">
                        ...
                        <div>
                            ...
                            <select id="reg-cohort" name="cohort_id"
                                ...
                                @foreach ($cohorts as $cohort)
                                    @php $spots = $cohort->spotsLeft(); $full = $spots === 0; @endphp
                        ...
                        <div>
                            <label for="reg-instrument" class="text-sm font-medium text-ink-900">Preferred instrument / track</label>
                            <select id="reg-instrument" name="instrument_interest"
                                class="mt-1 w-full rounded-xl border border-ink-200 bg-white px-4 py-3 text-sm text-ink-900 focus:ring-2 focus:ring-sage-400 focus:border-sage-400">
                                <option value="Sol-Fa & Choir" {{ old('instrument_interest') == 'Sol-Fa & Choir' ? 'selected' : '' }}>Sol-Fa & Choir</option>
                                <option value="Staff Notation" {{ old('instrument_interest') == 'Staff Notation' ? 'selected' : '' }}>Staff Notation</option>
                                <option value="Piano" {{ old('instrument_interest') == 'Piano' ? 'selected' : '' }}>Piano</option>
                                <option value="Guitar" {{ old('instrument_interest') == 'Guitar' ? 'selected' : '' }}>Guitar</option>
                                <option value="Violin" {{ old('instrument_interest') == 'Violin' ? 'selected' : '' }}>Violin</option>
                            </select>
                        </div>
                        <div>
                            <label for="reg-message" class="text-sm font-medium text-ink-900">Message (optional)</label>
                            <textarea id="reg-message" name="message" rows="3" maxlength="1000" placeholder="e.g. I’d like to start with Sol-Fa."
                        ...
                        <div class="flex flex-wrap gap-3 pt-2">
                            <button type="button" class="registration-prev rounded-xl border border-ink-200 bg-white px-5 py-3 text-sm font-semibold text-ink-700 hover:bg-ink-50 focus:outline-none focus:ring-2 focus:ring-sage-500 focus:ring-offset-2" data-prev="1">Back</button>
                            <button type="button" class="registration-next rounded-xl bg-sage-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-sage-700 focus:outline-none focus:ring-2 focus:ring-sage-500 focus:ring-offset-2" data-next="3">Continue to review</button>
                        </div>
                    </div>
                    ...
                    <div id="registration-step-3" class="registration-step-panel hidden space-y-4" data-step="3">
                        ...
                        <div class="rounded-2xl border border-ink-100 bg-white p-5 space-y-3">
                            <p class="text-sm text-ink-600"><strong class="text-ink-900">Name:</strong> <span id="review-name">—</span></p>
                            <p class="text-sm text-ink-600"><strong class="text-ink-900">Contact:</strong> <span id="review-contact">—</span></p>
                            <p class="text-sm text-ink-600"><strong class="text-ink-900">Cohort:</strong> <span id="review-cohort">—</span></p>
                            <p class="text-sm text-ink-600"><strong class="text-ink-900">Track:</strong> <span id="review-instrument">—</span></p>
                            <p class="text-sm text-ink-600" id="review-message-wrap" style="display:none"><strong class="text-ink-900">Message:</strong> <span id="review-message"></span></p>
                        </div>
                        <div class="flex flex-wrap gap-3 pt-2">
                            <button type="button" class="registration-prev rounded-xl border border-ink-200 bg-white px-5 py-3 text-sm font-semibold text-ink-700 hover:bg-ink-50 focus:outline-none focus:ring-2 focus:ring-sage-500 focus:ring-offset-2" data-prev="2">Back</button>
                            <button type="submit" class="rounded-xl bg-sage-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-sage-700 focus:outline-none focus:ring-2 focus:ring-sage-500 focus:ring-offset-2">
                                Submit registration
                            </button>
                        </div>
                    </div>
    ...
    (function () {
        ...
        function showStep(step) {
            ...
            indicators.forEach(function (el) {
                ...
                el.classList.remove('bg-sage-600', 'bg-sage-100', 'text-white', 'text-sage-700', 'border-2', 'border-ink-200', 'text-ink-400');
            ...
            document.getElementById('step-indicator-' + s)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }
        ...
        form.querySelectorAll('.registration-next').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (next === 2) {
                    ...
                    if (!phone && !email) {
                        var contactMsg = form.querySelector('[data-contact-hint]') || document.createElement('p');
                        if (!contactMsg.dataset.contactHint) {
                            ...
                            contactMsg.textContent = 'Please provide at least one of phone or email so we can confirm your registration.';
                            form.querySelector('#registration-step-1')?.appendChild(contactMsg);
                        }
                        (form.querySelector('#reg-phone') || form.querySelector('#reg-email'))?.focus();
                        return;
                    }
                }
                if (next === 3) {
                    ...
                    document.getElementById('review-name').textContent = (form.querySelector('#reg-name')?.value || '').trim() || '—';
                    var ph = (form.querySelector('#reg-phone')?.value || '').trim();
                    var em = (form.querySelector('#reg-email')?.value || '').trim();
                    document.getElementById('review-contact').textContent = [ph, em].filter(Boolean).join(' • ') || '—';
                    document.getElementById('review-cohort').textContent = cohortText;
                    document.getElementById('review-instrument').textContent = (form.querySelector('#reg-instrument')?.options[form.querySelector('#reg-instrument')?.selectedIndex]?.textContent || '').trim() || '—';
